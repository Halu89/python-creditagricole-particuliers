...
class DeferredOperations:
    ...
    def get_operations(self):
        ...
        url += "?grandeFamilleCode=%s&compteIdx=%s&carteIdx=%s" % (
        self.grandeFamilleCode, self.compteIdx, self.carteIdx)
...
class Operations:
    ...
    def get_operations(self, count, startIndex=None, limit=30, sleep=None):
        ...
        url = "%s" % self.session.url
        url += "/%s/particulier/operations/synthese/detail-comptes/" % self.session.regional_bank_url
        url += "jcr:content.n3.operations.json?grandeFamilleCode=%s&compteIdx=%s" % (
        self.grandeFamilleCode, self.compteIdx)
        url += "&idDevise=EUR"
